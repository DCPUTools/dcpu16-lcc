...
static token_list * build_token_list_f(FILE *fp)
{
    ...
    while (curr_char != EOF)
    {
        ...
        if (isalpha(curr_char) || curr_char == '_')
        {
            ...
            while (curr_char != EOF && (isalnum(curr_char) || curr_char == '_')
                    && ident_pos < IDENTBUF_SIZE);
            result_end->next = build_token_ident(ident_pos, current_line,
                    current_col);
        ...
        else if (isdigit(curr_char))
        {
            ...
            if (curr_char == 'x')
            {
                ...
                if (!isxdigit(curr_char))
                {
                    fprintf(
                            stderr,
                            "ERROR: Expected hexadecimal number (Line: %d, Col: %d)\n",
                ...
                {
                    while (curr_char != EOF && isxdigit(curr_char)
                            && ident_pos < IDENTBUF_SIZE)
                    {
                        ident_buf[ident_pos] = curr_char;
                        ident_pos++;
                ...
                result_end->next = build_token_hex(ident_pos, current_line,
                        current_col);
                result_end = result_end->next;
            }
            /* TODO: Binary */
            ...
            {
                while (curr_char != EOF && isdigit(curr_char)
                        && ident_pos < IDENTBUF_SIZE)
                {
                    ident_buf[ident_pos] = curr_char;
                    ident_pos++;
                ...
                result_end->next = build_token_dec(ident_pos, current_line,
                        current_col);
                result_end = result_end->next;
            }
            current_col += ident_pos;
        ...
        else if (curr_char == '\"')
        {
            ...
            while (curr_char != '\"' && curr_char != EOF)
            {
                ...
                if (ident_pos >= IDENTBUF_SIZE)
                {
                    ...
                    result_end->next = build_token_ident(ident_pos,
                            current_line, current_col);
                    result_end = result_end->next;
                    result_end->type = token_string;
                    /* Insert comma */
                    result_end->next = build_token(token_comma, current_line,
                            current_col);
                    result_end = result_end->next;
        ...
        {
            result_end->next = build_token(token_colon, current_line,
                    current_col);
            result_end = result_end->next;
        }
        /* Left bracket */
        ...
        {
            result_end->next = build_token(token_leftbracket, current_line,
                    current_col);
            result_end = result_end->next;
        }
        /* Right bracket */
        ...
        {
            result_end->next = build_token(token_rightbracket, current_line,
                    current_col);
            result_end = result_end->next;
        }
        /* Plus */
        ...
        {
            result_end->next = build_token(token_plus, current_line,
                    current_col);
            result_end = result_end->next;
        }
        /* White space */
        ...
        {
            fprintf(stderr,
                    "ERROR: Lexer failed on input '%c' (Line: %d, Col: %d)\n",
...
static enum op_code_nonbasic get_op_code_nonbasic(token_list *tok)
{
    switch (tok->type)
    {
        ...
        default:
            fprintf(
                    stderr,
                    "ERROR: Inavlid non-basic operator at line %d, column %d.\n",
...
static instruction new_instruction_basic(enum op_code o, enum op_value b,
        enum op_value a)
{
    instruction result;
    result.tag = basic;
...
static instruction new_instruction_nonbasic(enum op_code_nonbasic o,
        enum op_value a)
{
    instruction result;
    result.tag = nonbasic;
...
static label_loc_map * insert_into_label_loc_map(label_loc_map *map, char *name,
        int loc, token_list *tok)
{
...
static instruction_list * insert_literal(instruction_list *list,
        enum op_value val, word lit)
{
...
static int parse_label(instruction_list *list, token_list **tp,
        label_loc_map **ltp)
{
    token_list *tok;
    label_loc_map *match;
    ...
    if (!tok->next)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Expected label, found EOF at line %d, column %d.\n",
                tok->line_number, tok->col_number);
        return -1;
    }
    tok = tok->next;
    ...
    {
        fprintf(stderr,
                "ERROR: Syntax error: Expected label at line %d, column %d.\n",
                tok->line_number, tok->col_number);
        return -1;
    }
    ...
    if (match)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Duplicate labels at line %d, column %d and line %d, column %d.\n",
                match->tok->line_number, match->tok->col_number,
                tok->line_number, tok->col_number);
        return -1;
    }
...
static int parse_bracket(token_list **tp, label_loc_map **utp,
        enum op_value *val, word *lit, int loc)
{
    enum token type;
    if (!(*tp)->next)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Unexpected EOF after '[' at line %d, column %d.\n",
    ...
    *tp = (*tp)->next;
    ...
    if ((*tp)->type == token_number || (*tp)->type == token_label)
    {
        ...
        if (!(*tp)->next)
        {
            fprintf(
                    stderr,
                    "ERROR: Syntax error: Unexpected EOF after number at line %d, column %d.\n",
                    (*tp)->line_number, (*tp)->col_number);
            return -1;
        }
        /* [Lit + Reg] */
            ...
            *tp = (*tp)->next;
            switch ((*tp)->type)
            {
                ...
                default:
                    fprintf(
                            stderr,
                            "ERROR: Syntax error: Expected register at line %d, column %d.\n",
                            (*tp)->line_number, (*tp)->col_number);
                    return -1;
            }
        }
    ...
    else
    {
        ...
        if (!(*tp)->next)
        {
            fprintf(
                    stderr,
                    "ERROR: Syntax error: Unexpected EOF after register at line %d, column %d.\n",
                    (*tp)->line_number, (*tp)->col_number);
            return -1;
        }
        if ((*tp)->next->type == token_plus)
        {
            ...
            *tp = (*tp)->next;
            ...
            else
            {
                fprintf(
                        stderr,
                        "ERROR: Syntax error: Expected number or label at line %d, column %d.\n",
                        (*tp)->line_number, (*tp)->col_number);
                return -1;
            }
            switch (type)
    ...
    if (!(*tp)->next)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Unexpected EOF, expected ']' after token at line %d, column %d.\n",
                (*tp)->line_number, (*tp)->col_number);
        return -1;
    }
    *tp = (*tp)->next;
    ...
    {
        fprintf(stderr,
                "ERROR: Syntax error: Expected ']' at line %d, column %d.\n",
                (*tp)->line_number, (*tp)->col_number);
        return -1;
    }
    /* Success */
...
static int parse_value(token_list **tp, label_loc_map **utp, enum op_value *val,
        word *lit, int loc)
{
    if (!(*tp))
    {
...
static int parse_basic(instruction_list *list, token_list **tp,
        label_loc_map **utp)
{
    ...
    if (!(*tp)->next)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Unexpected EOF after line %d, column %d.\n",
                (*tp)->line_number, (*tp)->col_number);
        return -1;
    }
    *tp = (*tp)->next;
    ...
    *tp = (*tp)->next;
    ...
    else if (!(*tp)->next)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Expected token after ',' at line %d, column %d.\n",
                (*tp)->line_number, (*tp)->col_number);
        return -1;
    }
    else
...
static int parse_nonbasic(instruction_list *list, token_list **tp,
        label_loc_map **utp)
{
    enum op_value val;
    word lit;
...
static int parse_dat(instruction_list *list, token_list **tp,
        label_loc_map **utp)
{
    char *c;
    do
    {
        if (!(*tp)->next)
        {
            fprintf(
                    stderr,
                    "ERROR: Syntax error: Unexpected EOF in DAT field after line %d, column %d.\n",
                    (*tp)->line_number, (*tp)->col_number);
            return -1;
        }
        *tp = (*tp)->next;
        ...
        else if ((*tp)->type == token_label)
        {
            ...
            *utp = insert_into_label_loc_map(*utp, (*tp)->ident,
                    get_last_instr_loc(list), *tp);
        }
        ...
        else
        {
            fprintf(
                    stderr,
                    "ERROR: Syntax error: Expected number or label in DAT field at line %d, column %d.\n",
                    (*tp)->line_number, (*tp)->col_number);
            return -1;
        }
...
static label_loc_map * update_label(instruction_list *list,
        label_loc_map *to_update, label_loc_map *symbols)
{
    ...
    if (!entry)
    {
        fprintf(
                stderr,
                "ERROR: Syntax error: Non-existant label '%s' referenced at line %d, column %d.\n",
                to_update->name, to_update->tok->line_number,
                to_update->tok->col_number);
    ...
    while (tok)
    {
        switch (tok->type)
        {
            ...
            default:
                fprintf(
                        stderr,
                        "ERROR: Syntax error: Invalid start of statement at line %d, column %d.\n",
                        tok->line_number, tok->col_number);
                return NULL;
        }
        tok = tok->next;
