...
static int requate(Node q)
{
    ...
    debug(
            fprint(stderr, "(requate(%x): tmp=%s src=%s)\n", q, tmp->x.name, src->x.name));
    for (p = q->x.next; p; p = p->x.next)
        if (p->x.copy && p->syms[RX] == src && p->x.kids[0]->syms[RX] == tmp)
            ...
            break;debug(fprint(stderr, "(requate arm 7 at %x)\n", p));
    assert(n > 0);
    for (p = q->x.next; p; p = p->x.next)
...
static Symbol spillee(Symbol set, unsigned mask[], Node here)
{
    ...
    assert(bestreg->x.regnode->vbl == NULL);
...
static void genspill(Symbol r, Node last, Symbol tmp)
{
    ...
    for (p = last->x.next; p != q; p = p->x.next)
    {
        ralloc(p);
        assert(
                !p->x.listed || !NeedsReg[opindex(p->op)] || !(*IR->x.rmap)(opkind(p->op)));
    }
}
...
static void genreload(Node p, Symbol tmp, int i)
{
    ...
    debug(
            fprint(stderr, "(replacing %x with a reload from %s)\n", p->x.kids[i], tmp->x.name));
...
static int reprune(Node *pp, int k, int n, Node p)
{
    ...
    {
        debug(
                fprint(stderr, "(reprune changes %x from %x to %x)\n", pp, *pp, p->x.kids[n]));
        *pp = p->x.kids[n];
        x = *p;
        (IR->x.target)(&x);
    }
...
void spill(unsigned mask, int n, Node here)
{
    ...
    if (mask & ~freemask[n])
    {
        ...
        assert(
                /* It makes no sense for a node to clobber() its target. */
                here->x.registered == 0 || /* call isn't coming through clobber() */
                here->syms[RX] == NULL || here->syms[RX]->x.regnode == NULL || here->syms[RX]->x.regnode->set != n || (here->syms[RX]->x.regnode->mask&mask) == 0);
