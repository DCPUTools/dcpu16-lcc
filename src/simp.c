...
Tree simplify(int op, Type ty, Tree l, Tree r)
{
    ...
    switch (op)
    {
        ...
        case CVU + I:
            ...
            break;
        case CVP + U:
            xcvtcnst(P, (unsigned long)l->u.v.p, ty, u,
                    (unsigned long)l->u.v.p);
            break;
        case CVU + P:
        ...
        case CVI + U:
            xcvtcnst(I, l->u.v.i, ty, u,
                    ((unsigned long)l->u.v.i)&ones(IR->byte_width*ty->size));
            break;
        case CVU + U:
            xcvtcnst(U, l->u.v.u, ty, u,
                    l->u.v.u&ones(IR->byte_width*ty->size));
            break;
        ...
        case ADD + P:
            ...
            if (l->op == ADDRG + P && l->u.sym->generated
                    && (r->op == CNST + I
                            && (r->u.v.i > 32767 || r->u.v.i < -32768)
                            || r->op == CNST + U && r->u.v.u > 65536)
                    || IR->byte_width != 8) //ugly hack to work around assemblers with NO offset support
                break;
            ...
            if (l->op == RIGHT && l->kids[1])
                ...
                return tree(RIGHT, ty, simplify(ADD + P, ty, l->kids[0], r),
                        NULL);
            break;
        ...
        case BCOM + I:
            ufoldcnst(I,
                    cnsttree(ty, (long)extend((~l->u.v.i)&ones(IR->byte_width*ty->size), ty)));
            idempotent(BCOM+U);
            break;
        case BCOM + U:
            ufoldcnst(U,
                    cnsttree(ty, (unsigned long)((~l->u.v.u)&ones(IR->byte_width*ty->size))));
            idempotent(BCOM+U);
            break;
        case BOR + U:
            foldcnst(U, u, |);
        ...
        case SUB + P:
            if (l->op == CNST + P && r->op == CNST + P)
                return cnsttree(ty,
                        (long) ((char *) l->u.v.p - (char *) r->u.v.p));
            if (r->op == CNST + I || r->op == CNST + U)
                return simplify(ADD, ty, l,
                        cnsttree(inttype,
                                r->op == CNST + I ?
                                        -r->u.v.i : -(long) r->u.v.u));
            if (isaddrop(l->op) && r->op == ADD + I
                    && r->kids[1]->op == CNST + I)
                /* l - (x + c) => l-c - x */
